...
describe('Menu', () => {
  it('should accept defaultOpenKeys in mode inline', () => {
    ...
    expect(
      container
        .querySelector('.rc-design-menu-submenu-open')
        ?.querySelector('.rc-design-menu-submenu-title')?.textContent,
    ).toEqual('submenu1');
  });
  ...
  it('If has select nested submenu item ,the menu items on the grandfather level should be highlight', () => {
    ...
    expect(
      container.querySelectorAll('li.rc-design-menu-submenu-selected').length,
    ).toBe(1);
  });
});
